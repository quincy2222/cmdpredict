...
function trade(){
  ...
  logActivity({type:'trade',user:S.name,marketId:S.sel.id,amount:a,side:S.side,outcome:oc.label,
    desc:`${S.name} bought ${shares} ${S.side.toUpperCase()} "${oc.label}" at ${Math.round(price*100)}¢ for $${a}`});
  flash(`Bought ${shares} ${S.side.toUpperCase()} "${oc.label}" at ${Math.round(price*100)}¢`);
}
...
function mountDetail(){
  ...
  function updatePreview(){
    ...
    if(a>0&&m.outcomes[s.selOc]){
      ...
      if(preview)preview.innerHTML=`<div style="display:flex;justify-content:space-between"><span>${sh} shares @ ${Math.round(pr*100)}¢</span><span style="color:${GN};font-weight:600">Payout: $${po}</span></div>`;
